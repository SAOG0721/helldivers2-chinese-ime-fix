@echo off
chcp 437 >nul
title Remove Chinese language and set English as preferred

set "PS1=%TEMP%\remove_chinese_language_%RANDOM%.ps1"
...
>> "%PS1%" echo }
>> "%PS1%" echo Write-Host ""
>> "%PS1%" echo $target = 'en-US'
>> "%PS1%" echo Write-Host "Target preferred language: English (United States) / en-US"
>> "%PS1%" echo $newList = New-WinUserLanguageList $target
>> "%PS1%" echo foreach ($lang in $current) {
>> "%PS1%" echo     if ($lang.LanguageTag -eq $target) { continue }
>> "%PS1%" echo     if ($lang.LanguageTag -like 'zh*') {
>> "%PS1%" echo         Write-Host ("Removing Chinese language entry: " + $lang.LanguageTag)
>> "%PS1%" echo         continue
>> "%PS1%" echo     }
